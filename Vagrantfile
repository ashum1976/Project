...
## Установка на сервер и бэкапный сервер mysql-a версии 4.0.0 ansible
      if boxname.to_s == "srvmysql"
        ...
        box.vm.provision "shell", inline: <<-SHELL
          ...
          dnf config-manager --set-enabled powertools
	        dnf -y --nogpgcheck install python39 epel-release http://repo.zabbix.com/zabbix/5.4/rhel/8/x86_64/zabbix-release-5.4-1.el8.noarch.rpm
      ...
      elsif boxname.to_s == "backupmysql"
...
##################################3
          ...
          systemctl restart sshd
          dnf config-manager --set-enabled powertools
          dnf -y --nogpgcheck install epel-release http://repo.zabbix.com/zabbix/5.4/rhel/8/x86_64/zabbix-release-5.4-1.el8.noarch.rpm
	        dnf -y --nogpgcheck install sshpass
